import owen
import random
#Chaque bateau est défini par : son nombre de cases, son sens,ses coordonnées et son code
bateauxj = [[5,0,[0,0],5], [4,0 ,[0,0],4], [3,0,[0,0],3], [3,0,[0,0],2], [2,0,[0,0],1]]               #porte_avion, croiseur, contre_torpilleur_1, contre_torpilleur_2, torpilleur 
bateaux_ia = bateauxj
def ecran():
    a = 'ABCDEFGHIJ'
    ecran = [[[k + str(i + 1), 0, 0] for k in a] for i in range(10)]
    return ecran
#Le but de case est que... -on veut entendre la suite, clem !!!-owen
def case(bateaux_ia) :
    ...
    for i in range(len(bateauxj)) :
        ...
        while z == 0 :
          try:
                ...
                if 1 <= int(coordonnées_liste[1]) <= 10 and 0 <= sens <= 1:
                    if ((int(coordonnées_liste[1]) + bateauxj[i][0] <= 11) and (sens == 1)) or (coordonnées_liste[0] + bateauxj[i][0] <= 10) and (sens == 0):
                        z = 1
                    else :
                        raise TypeError
                else :
